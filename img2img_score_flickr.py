...
if __name__ == "__main__":
    ...
    experiment_name = 'img2img_captions_latent_guidance_scale_7.5_strength_0.8_steps_50_img2img_seed_2'
    ...
    for i, (caption, (correct_path, all_images)) in tqdm(enumerate(top10.items()), total=len(top10.items())):
        ...
        for j in range(10):
            ...
            if USE_CLIP:
                image_after = Image.open(f'flickr/{experiment_name}/{i}_{img_id}.png').convert('RGB')
                image_after = preprocess(image_after).unsqueeze(0).to(device)
                with torch.no_grad():
                    latent_after = clip_model.encode_image(image_after).squeeze().float()
            else:
                latent_after = torch.load(f'flickr/{experiment_name}/{i}_{img_id}.pt', map_location=torch.device(device)).flatten().float()
