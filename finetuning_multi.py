...
def main():
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        for kk, prompt in enumerate(args.validation_prompt):
                            ...
                            save_path = os.path.join(save_path, f"validation_{kk}.png")
    ...
    generator = torch.Generator(device=accelerator.device).manual_seed(args.seed)
    images = []
    for kk, prompt in enumerate(args.final_inference_prompt):
        image = pipeline(prompt, num_inference_steps=30, generator=generator).images[0]
        # save img to output folder with prompt written on image at top
        ...
        save_path = os.path.join(args.output_dir, f"final_inference_{kk}.png")
        cv2.imwrite(save_path, image)
        images.append(image)
